# use beam seach to decode the test datasets
...
class Beam(object):
    ...
    def extend(self, token, log_prob, state, context):
        # update the beam object, with adding the tokens and
        # log probs but update the state and context
...
class BeamSearch(object):
    ...
    def beam_search(self, batch):
        ...
        while steps < config.max_dec_steps and len(results) < config.beam_size:
            ...
            topk_log_probs, topk_ids = torch.topk(final_dist, config.beam_size * 2) # why double the beam size here
